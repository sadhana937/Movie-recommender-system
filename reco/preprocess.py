...
def encode_user_item(df, user_col, item_col, rating_col, time_col):
    ...
    encoded_ratings = pd.DataFrame({
        'USER': encoded_df["USER"],
        'ITEM': encoded_df["ITEM"],
        'RATING': encoded_df["RATING"],  
        'TIMESTAMP': encoded_df["TIMESTAMP"]
    })
    return encoded_ratings # user_encoder, item_encoder

def encode_user_item_withencoder(df, user_col, item_col, rating_col, time_col):
    """
    Function to encode users and items
